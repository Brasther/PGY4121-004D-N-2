...
export class LoginPage implements OnInit {
  ...
  constructor(
    ...
    this.formularioLogin = this.fb.group({
      'usuario': new FormControl("", [Validators.required, Validators.pattern(/^[a-zA-Z0-9]{3,8}$/)]),
  ...
  async ingresar() {
    if (this.formularioLogin.valid) {
      ...
      const usuarios: { usuario: string, password: string }[] = usuariosJSON && usuariosJSON.value ? JSON.parse(usuariosJSON.value) : [];
  
      // Busca al usuario por su nombre de usuario (campo 'usuario')
      const user = usuarios.find((u: any) => u.usuario === f.usuario && u.password === f.password);
  
      if (user) {
        // Guarda los datos del usuario que ha iniciado sesión
        await Preferences.set({ key: 'nombreUsuario', value: user.usuario });
